...
namespace E_commerce.Data
{
    public class E_commerceDB
    {
        ...
        {
            var connectionString = _configuration.GetConnectionString("OrderServiceConnection")
            ...
            if (string.IsNullOrWhiteSpace(connectionString))
            {
                throw new InvalidOperationException(
                    "Connection string is missing. Configure ConnectionStrings:OrderServiceConnection, ConnectionStrings:VideoGame, or ConnectionStrings:DefaultConnection in appsettings.json.");
            }
        ...
        public async Task EnsureDatabaseObjectsAsync()
        {
            ...
            await connection.ExecuteAsync(dropLegacyForeignKeySql);
            await connection.ExecuteAsync(orderUserIdIndexSql);
            await connection.ExecuteAsync(productSql);
            await connection.ExecuteAsync(OrderIteamSql);
